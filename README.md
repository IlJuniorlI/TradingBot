...
- **Structured JSON**: `SESSION_REPORT {...}` at TRADEFLOW level. Carries the same aggregates as first-class keys: `per_regime`, `per_symbol`, `per_exit_reason`, `per_hour`, `mae_mfe`, `post_stop_continuation`, `entry_timing`, `filter_rejections`. Any downstream tool (dashboard, spreadsheet import, analyzer) can consume it directly without scraping the human log.
- **Gate attribution** (`manifest.json` only, not the log): for every SKIPPED decision, the net forward price move over 30 minutes toward the side the bot was about to take, in ATR, bucketed by skip reason and split by regime. Each entry carries `blocked`, `median_net_atr`, `favourable_pct` and the regimes that produced it; `costliest_gates` ranks by blocks x edge above baseline.

  The **baseline is sampled from the same session's bars** and is what makes the numbers readable — a trending day lifts every LONG-side figure, so only the gap above baseline is evidence. The excursion fields beside the net move are near chance at this window (77% of random 30-minute windows touch 1 ATR up, because a 1m ATR-14 measures fourteen minutes of range) and are kept only because a wide favourable excursion against a flat net says "it went our way and came back" — a stop-placement story rather than an entry one.
