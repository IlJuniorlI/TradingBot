...
- **Structured JSON**: `SESSION_REPORT {...}` at TRADEFLOW level. Carries the same aggregates as first-class keys: `per_regime`, `per_symbol`, `per_exit_reason`, `per_hour`, `mae_mfe`, `post_stop_continuation`, `filter_rejections`. Any downstream tool (dashboard, spreadsheet import, analyzer) can consume it directly without scraping the human log.
