...
class DashboardCache:
    ...
    def symbol_snapshot(
        ...
        *,
        ...
        # Resolve disagreements between the SR sidebar (sr_row) and the chart
        # zone classifier (key_level_zones). Both are derived from HTF data
        # but with different flip thresholds: `support_resistance.build_…`
        # confirms a flip when price clears the LEVEL price by `flip_eps`
        # (~1e-6 of close — effectively the level itself), while
        # `zone_flip_confirmed` (used by `strategy_level_zones`) confirms
        # against the zone TOP / BOTTOM (level ± zone_half_width). When
        # price sits inside a zone band, the sidebar can call a level
        # "support" while the chart still renders the band as
        # "resistance pending flip" — visually a green Support line lands
        # inside a red Resistance zone.
        #
        # We resolve in favor of the chart zone (which has the stricter
        # band-clearance semantic and matches what position management's
        # ladder logic uses): drop any sidebar Support price that falls
        # strictly inside a current resistance-classified zone, and any
        # sidebar Resistance price strictly inside a support-classified
        # zone. We also drop nearest_/next_ if they hit the same condition,
        # and update the sr_row dict so the dashboard sidebar and the chart
        # agree on what the Support / Resistance ladder looks like. The
        # underlying SupportResistanceContext used by strategies isn't
        # mutated — strategies still see the full level set.
        zone_resistance_bands: list[tuple[float, float]] = []
        zone_support_bands: list[tuple[float, float]] = []
        for zone in (key_level_zones or []):
            try:
            ...
            if kind == "resistance":
                zone_resistance_bands.append((lower, upper))
            elif kind == "support":
                zone_support_bands.append((lower, upper))

        def _price_inside_band(price: float | None, bands: list[tuple[float, float]]) -> bool:
            if price is None:
                return False
            try:
                value = float(price)
            ...
            if value <= 0:
                return False
            return any(lower < value < upper for lower, upper in bands)

        if zone_resistance_bands:
            support_prices = [p for p in support_prices if not _price_inside_band(p, zone_resistance_bands)]
            if _price_inside_band(nearest_support, zone_resistance_bands):
                nearest_support = support_prices[0] if support_prices else None
            if _price_inside_band(next_support, zone_resistance_bands):
                next_support = next((p for p in support_prices if nearest_support is None or abs(p - nearest_support) > max(1e-9, ladder_min_gap)), None)
        if zone_support_bands:
            resistance_prices = [p for p in resistance_prices if not _price_inside_band(p, zone_support_bands)]
            if _price_inside_band(nearest_resistance, zone_support_bands):
                nearest_resistance = resistance_prices[0] if resistance_prices else None
            if _price_inside_band(next_resistance, zone_support_bands):
                next_resistance = next((p for p in resistance_prices if nearest_resistance is None or abs(p - nearest_resistance) > max(1e-9, ladder_min_gap)), None)
        if sr_row is not None and (zone_resistance_bands or zone_support_bands):
            sr_row = dict(sr_row)
            if zone_resistance_bands:
                sr_row["supports"] = [p for p in (sr_row.get("supports") or []) if not _price_inside_band(p, zone_resistance_bands)]
                if _price_inside_band(sr_row.get("nearest_support"), zone_resistance_bands):
                    sr_row["nearest_support"] = nearest_support
            if zone_support_bands:
                sr_row["resistances"] = [p for p in (sr_row.get("resistances") or []) if not _price_inside_band(p, zone_support_bands)]
                if _price_inside_band(sr_row.get("nearest_resistance"), zone_support_bands):
                    sr_row["nearest_resistance"] = nearest_resistance
