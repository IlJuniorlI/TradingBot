...
| `--tint-rgb`                     | RGB triplet (no alpha) for theme-aware surface tints used across `dashboard.css` (subtle highlights, dividers, hover borders, gauge ring tracks, chart-wrap radial-masked grid overlay) and the chart time-axis tick marks in `dashboard.js`. Dark themes use `255, 255, 255` (white tint on dark surfaces); light theme uses `15, 23, 42` (slate tint on light surfaces). Each call site picks its own opacity. |
