...
class RTHTrendPullbackStrategy(BaseStrategy):
    ...
    def entry_signals(self, candidates: list[Candidate], bars: dict[str, pd.DataFrame], positions: dict[str, Position], client=None, data=None) -> list[Signal]:
        ...
        for c in candidates:
            ...
            trigger_high = self._safe_float(trigger_slice["high"].max(), last_close)
            trigger_low = self._safe_float(trigger_slice["low"].min(), last_close)
            support_low = self._safe_float(support_slice["low"].min(), last_close)
            resistance_high = self._safe_float(support_slice["high"].max(), last_close)
            pullback_low = self._safe_float(pullback_slice["low"].min(), last_close)
            pullback_high = self._safe_float(pullback_slice["high"].max(), last_close)
            ...
            if directional_bias == Side.LONG:
                ...
                if day_strength < min_change:
                    reasons.append(self._reason_with_values("weak_day_strength", current=day_strength, required=min_change, op=">=", digits=4))
                ...
                if last_ret5 < trend_min_ret5:
                    reasons.append(self._reason_with_values("weak_ret5", current=last_ret5, required=trend_min_ret5, op=">=", digits=4))
                if last_ret15 < trend_min_ret15:
                    reasons.append(self._reason_with_values("weak_ret15", current=last_ret15, required=trend_min_ret15, op=">=", digits=4))
                if extension_pct > max_extension:
                    reasons.append(self._reason_with_values("too_extended_from_vwap", current=extension_pct, required=max_extension, op="<=", digits=4))
                if not pullback_hold_ok:
                    reasons.append(self._reason_with_values("pullback_lost_support", current=pullback_low, required=long_support_ref * (1.0 - support_hold_pct), op=">=", digits=4))
                if not bullish_trigger:
                    reasons.append(self._reason_with_values("no_reexpansion_trigger", current=last_close, required=trigger_high, op=">", digits=4))
                if close_pos < min_bar_close_position:
                    reasons.append(self._reason_with_values("weak_bar_close", current=close_pos, required=min_bar_close_position, op=">=", digits=4))
                ...
                if day_strength > -min_change:
                    reasons.append(self._reason_with_values("weak_day_weakness", current=day_strength, required=-min_change, op="<=", digits=4))
                if last_close >= last_vwap:
                    reasons.append(self._reason_with_values("above_vwap", current=last_close, required=last_vwap, op="<", digits=4))
                if last_ema9 > last_ema20:
                    reasons.append(self._reason_with_values("ema9_above_ema20", current=last_ema9, required=last_ema20, op="<=", digits=4))
                if last_ret5 > -trend_min_ret5:
                    reasons.append(self._reason_with_values("weak_ret5", current=last_ret5, required=-trend_min_ret5, op="<=", digits=4))
                if last_ret15 > -trend_min_ret15:
                    reasons.append(self._reason_with_values("weak_ret15", current=last_ret15, required=-trend_min_ret15, op="<=", digits=4))
                if extension_pct > max_extension:
                    reasons.append(self._reason_with_values("too_extended_from_vwap", current=extension_pct, required=max_extension, op="<=", digits=4))
                if not pullback_hold_ok:
                    reasons.append(self._reason_with_values("bounce_lost_resistance", current=pullback_high, required=short_res_ref * (1.0 + support_hold_pct), op="<=", digits=4))
                if not bearish_trigger:
                    reasons.append(self._reason_with_values("no_reexpansion_trigger", current=last_close, required=trigger_low, op="<", digits=4))
                if close_pos > (1.0 - min_bar_close_position):
                    reasons.append(self._reason_with_values("weak_bar_close", current=close_pos, required=1.0 - min_bar_close_position, op="<=", digits=4))
