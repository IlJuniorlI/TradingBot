...
class PeerConfirmedHTFPivotsStrategy(PeerConfirmedKeyLevelsStrategy):
    ...
    def dashboard_overlay_candidates(
        ...
        if ltf is None or ltf.empty:
            return []
        atr = max(
            self._optional_float(getattr(htf, "atr14", None)) or max(close * 0.0015, 0.01),
        ...
        def _candidate(level_obj, fallback_price: float | None, fallback_kind: str) -> dict[str, Any] | None:
            price = self._optional_float(getattr(level_obj, "price", None)) if level_obj is not None else fallback_price
        ...
        if side == Side.LONG:
            primary = _candidate(getattr(htf, "broken_resistance", None), self._optional_float(getattr(htf, "reference_low", None)), "bullish_sr_support")
            secondary = _candidate(getattr(htf, "nearest_support", None), self._optional_float(getattr(htf, "reference_low", None)), "bullish_sr_support")
        else:
            primary = _candidate(getattr(htf, "broken_support", None), self._optional_float(getattr(htf, "reference_high", None)), "bearish_sr_resistance")
            secondary = _candidate(getattr(htf, "nearest_resistance", None), self._optional_float(getattr(htf, "reference_high", None)), "bearish_sr_resistance")
    ...
    def dashboard_zone_width_for_level(
        ...
        candidate_width = None if not isinstance(candidate, dict) else self._optional_float(candidate.get("zone_width"))
        width = max(
    ...
    def _level_obj_metrics(self, level_obj: Any | None, fallback_kind: str) -> dict[str, Any] | None:
        if level_obj is None:
            return None
        price = self._optional_float(getattr(level_obj, "price", None))
        if price is None or price <= 0:
            return None
    ...
    def _regime_signal(self, side: Side, ltf: pd.DataFrame, sr_ctx) -> dict[str, Any]:
        ...
        atr = max(
            self._safe_float(last.get("atr14"), getattr(sr_ctx, "current_atr", None) or max(close * 0.0015, 0.01)),
            max(close * 0.0005, 0.01),
        )
        ...
        if battleground_price is None or battleground_price <= 0:
            ...
            if battleground_distance_atr is not None and battleground_distance_atr <= proximity_limit:
                ...
                score += 0.75
                reasons.append(self._reason_with_values("late_to_battleground_atr", current=battleground_distance_atr, required=proximity_limit, op="<=", digits=4))
            else:
                reasons.append(self._reason_with_values("too_far_from_battleground_atr", current=battleground_distance_atr, required=proximity_limit, op="<=", digits=4))
            ...
            if side == Side.LONG:
                if close >= (battleground_price - zone_width):
                    ...
                    reasons.append(self._reason_with_values("support_decisively_lost", current=close, required=battleground_price - zone_width, op=">=", digits=4))
            else:
                if close <= (battleground_price + zone_width):
                    score += 0.9
                else:
                    reasons.append(self._reason_with_values("resistance_decisively_lost", current=close, required=battleground_price + zone_width, op="<=", digits=4))
        ...
        if target_clearance_atr is not None and target_clearance_atr >= min_target_clearance_atr:
            ...
            reasons.append(self._reason_with_values("tight_target_clearance_atr", current=target_clearance_atr, required=min_target_clearance_atr, op=">=", digits=4))
        else:
    ...
    def _volume_ratio(self, ltf: pd.DataFrame, lookback: int = 5) -> float:
        if ltf is None or ltf.empty:
            return 0.0
        current = self._safe_float(ltf.iloc[-1].get("volume"), 0.0)
        prior = ltf.iloc[:-1]
        if prior.empty:
            return 0.0
        baseline = max(
            self._safe_float(prior["volume"].tail(max(2, lookback)).mean(), 0.0),
            1.0,
        )
    ...
    def _pivot_reclaim_signal(
        ...
        *,
        ...
        score = 0.0
        if side == Side.LONG:
            touched = self._safe_float(prior["low"].min(), close) <= (pivot + zone_width)
            swept = self._safe_float(prior["low"].min(), close) <= pivot
            ...
            if reclaimed:
                ...
                reasons.append(self._reason_with_values("pivot_not_reclaimed", current=close, required=pivot + reclaim_offset, op=">=", digits=4))
            ...
            if close >= vwap:
                score += 0.25
            stop_anchor = min(pivot, self._safe_float(prior["low"].min(), pivot))
        else:
            touched = self._safe_float(prior["high"].max(), close) >= (pivot - zone_width)
            swept = self._safe_float(prior["high"].max(), close) >= pivot
            reclaimed = close <= (pivot - reclaim_offset)
            if touched:
                score += 1.0
            else:
            ...
            if close <= vwap:
                score += 0.25
            stop_anchor = max(pivot, self._safe_float(prior["high"].max(), pivot))
        if close_pos >= min_close_pos if side == Side.LONG else close_pos <= (1.0 - min_close_pos):
            score += 0.5
        else:
            reasons.append(self._reason_with_values("weak_bar_close", current=close_pos, required=min_close_pos if side == Side.LONG else (1.0 - min_close_pos), op=">=" if side == Side.LONG else "<=", digits=4))
        if volume_ratio >= min_volume_ratio:
            score += 0.5
        else:
            reasons.append(self._reason_with_values("weak_trigger_volume", current=volume_ratio, required=min_volume_ratio, op=">=", digits=4))
        return {
    ...
    def _pivot_rejection_signal(
        ...
        *,
        ...
        volume_ratio = self._volume_ratio(ltf)
        min_volume_ratio = max(0.1, float(self.params.get("min_trigger_volume_ratio", 1.0)))
        upper_wick_frac, lower_wick_frac, _, _ = self._bar_wick_fractions(ltf)
        ...
        if side == Side.LONG:
            ...
            if held:
                ...
                reasons.append(self._reason_with_values("pivot_not_held", current=close, required=pivot, op=">=", digits=4))
            if reversal_candle:
                score += 0.75
            else:
    ...
    def _pivot_continuation_signal(
        ...
        *,
        ...
        if side == Side.LONG:
            trigger_level = self._safe_float(prior["high"].tail(max(3, interaction_lookback // 2)).max(), close)
            interacted = self._safe_float(interaction_slice["low"].min(), close) <= (pivot + (zone_width * 1.25))
            pivot_held = self._safe_float(prior["low"].min(), close) >= (pivot - zone_width)
            ...
            if interacted or self._pivot_distance_atr(side, close, pivot, atr) <= max_distance_atr:
                ...
                reasons.append(self._reason_with_values("too_far_from_pivot_atr", current=self._pivot_distance_atr(side, close, pivot, atr), required=max_distance_atr, op="<=", digits=4))
            if pivot_held:
                score += 1.0
            else:
            ...
            if trigger_broke:
                ...
                reasons.append(self._reason_with_values("continuation_trigger_not_broken", current=close, required=trigger_level * (1.0 + breakout_buffer_pct), op=">=", digits=4))
            if self._ltf_structure_supports_side(side, ms_ltf):
                score += 0.75
            else:
                reasons.append("ltf_structure_not_bullish")
            ...
            trigger_level = self._safe_float(prior["low"].tail(max(3, interaction_lookback // 2)).min(), close)
            interacted = self._safe_float(interaction_slice["high"].max(), close) >= (pivot - (zone_width * 1.25))
            pivot_held = self._safe_float(prior["high"].max(), close) <= (pivot + zone_width)
            trigger_broke = close <= (trigger_level * (1.0 - breakout_buffer_pct))
            if interacted or self._pivot_distance_atr(side, close, pivot, atr) <= max_distance_atr:
                score += 1.0
            else:
    ...
    def _build_pivot_signal(
        ...
        *,
        ...
        pivot_price = self._optional_float(regime.get("pivot_price"))
        ...
        if regime_score < min_regime_score:
            hard_reasons.append(self._reason_with_values("weak_regime_score", current=regime_score, required=min_regime_score, op=">=", digits=4))
        ...
        if trigger_score < min_trigger_score:
            hard_reasons.append(self._reason_with_values("weak_trigger_score", current=trigger_score, required=min_trigger_score, op=">=", digits=4))
        ...
        if directional_peer_score >= min_peer_score:
            ...
            hard_reasons.append(self._reason_with_values("weak_peer_score", current=directional_peer_score, required=min_peer_score, op=">=", digits=2))
        ...
        if total_score < min_total_score:
            hard_reasons.append(self._reason_with_values("weak_total_score", current=total_score, required=min_total_score, op=">=", digits=4))
        ...
        if distance_atr > max_distance:
            hard_reasons.append(self._reason_with_values("too_far_from_pivot_atr", current=distance_atr, required=max_distance, op="<=", digits=4))
        ...
        expansion_price = self._optional_float(regime.get("expansion_price"))
        target_level_price = self._optional_float(regime.get("target_level_price"))
        ...
        for candidate in candidates:
            ...
            for side in side_order:
                ...
                if isinstance(failure_payload, dict):
                    ...
                    prefixed = self._side_prefixed_reasons(side, failure_payload.get("reasons") or [failure_payload.get("primary_reason") or f"{side.value.lower()}_setup_not_ready"])
