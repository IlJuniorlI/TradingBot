...
- **Hard screener-bias veto** (gated by the existing `respect_screener_bias`). The Fix A soft penalty (section 6a) drags counter-bias scores down but doesn't outright veto. On normal 0.3-1% days, the penalty maxes at ~0.25 with `bias_penalty_saturate_at: 2.0` — too weak to filter `min_pullback_score: 3.5` candidates with raw scores 3.75+. The new hard gate skips a candidate entirely when `c.directional_bias` is set AND lies outside `preferred_sides` (e.g., screener tagged SHORT but `risk.allow_short: false`). Lives in `entry_signals` right after the trailing-bias fallback; same `respect_screener_bias` toggle as the soft penalty; same `orb_bypass_screener_bias` bypass during 09:35-`orb_end_time`.
...
- **`pullback_require_bounce_confirmation`** (default `true`). Pullback regime is meant to fire on a bounce completing, not mid-fall. Without this gate, `_build_pullback_signal` accepts entries where the candidate is still falling — FCX on 5/26 entered at 10:12:26 with the in-progress 5m bar showing close in the bottom 19% of the bar's range, lost $44. The gate requires (a) the entry bar's close in the directional half of the bar (close_pos ≥ `pullback_bounce_close_position_min`, default `0.5` — upper half for LONG, lower half for SHORT), AND (b) the bar close on the favorable side of the prior 5m bar's close. Both conditions must hold. Skipped automatically when `len(session_ltf) < 2` (first bar of session).
...
All gates are independently toggleable via the manifest (or `params` in `configs/config.top_tier_adaptive.yaml`). The hard screener-bias veto and stretched cooldown are gated by their existing parent flags (`respect_screener_bias`, `reject_stretched_entries`) so toggling those off disables the new gates too. The relative-strength gate has its own `relative_strength_block_threshold_pct: 0.0` off-switch (any zero/negative threshold disables it). The pullback bounce gate has `pullback_require_bounce_confirmation: false` to disable.

Modeled effect on the 5/26 entry batch with all four gates active: 5 of 7 trades blocked (NEM, INTC×2 by RS gate; AMZN by stretched cooldown; FCX#1 by pullback bounce), 2 of 7 enter (META +$10.40 winner, FCX#2 still loses -$38.87). Net: -$28.47 vs -$177.66 original (-84% loss reduction).
...
- `orb_bypass_oversized_entry_bar`: skip the bar-size gate during the ORB window. Default `true`. Opening flush bars are always large; the existing `orb_bypass_stretched_filter` handles that case.
...
- `orb_bypass_relative_strength`: skip the relative-strength gate during 09:35-`orb_end_time` (default `true`). Stock-vs-sector divergence is noisy in the first 30 minutes.
...
- `pullback_require_bounce_confirmation`: require pullback entries to show bar-close-in-favorable-half AND close-better-than-prior-bar before building (default `true`). Blocks the "pullback that's actually a rollover" pattern. See section 6f.
- `pullback_bounce_close_position_min`: close-position threshold for the pullback bounce gate (default `0.5`). LONG needs `close_pos ≥ threshold` (close in upper half); SHORT needs `close_pos ≤ 1 − threshold`.
