...
Each regime can be globally disabled via its own opt-out knob: `disable_trend_regime`, `disable_pullback_regime`, `disable_range_regime`, `disable_vol_squeeze_regime`, `disable_momentum_regime`, `disable_sr_scalp_regime` — all default `false`. The 7th regime, **orb** (the true Opening Range Breakout, sole regime in the opening window), is controlled by `disable_orb_window` instead.
