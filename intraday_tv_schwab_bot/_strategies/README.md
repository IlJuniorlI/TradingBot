...
- **Shared divergence entry/exit helpers (opt-in).** `BaseStrategy` exposes two helpers that any strategy can call from its signal pipeline to surface divergence-driven trade decisions without writing its own pivot detection:
  - `_divergence_entry_candidate(side, close, ltf, sr_ctx, tech_ctx)` returns `Optional[dict]` with `trigger_level`, `stop`, `target`, `score`, `reason`, `kind`, `indicator`, `age_bars`, `confluence_level`. Gated by `shared_entry.use_divergence_entry_signal` (off by default); requires the strategy author to actually call the helper from their signal logic. Stops anchor to the divergence pivot ± `technical_levels.stop_buffer_atr_mult * atr`; targets use the smaller of `nearest opposing S/R level` or `min_target_rr * risk`. Score floor + age gate + S/R-confluence gate are all configurable. (HTF-side divergence influences the score separately via `_htf_divergence_adjustment` — it doesn't enter this entry candidate's score directly.)
